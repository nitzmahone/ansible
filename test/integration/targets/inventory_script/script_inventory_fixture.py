...
inventory = dict(
    ...
    group1=dict(hosts=["host1"], vars=dict(a_group1_groupvar="avalue")),
    list_as_group=[],  # assigned to `hosts` in a new dict
    rewrite_as_host=dict(avar="value", untrusted_var=dict(__ansible_unsafe="untrusted value")),  # rewritten as a group with a host named `rewrite_as_host`
)

inventory_with_profile = copy.deepcopy(inventory)
inventory_with_profile['_meta']['profile'] = 'legacy_inventory'
...
normal_outputs = dict(
    ...
    no_meta_hostvars_host_nonzero_rc=dict(mygroup=dict(hosts=["myhost"])),  # triggers the legacy "slow" path which invokes the script with --host for each host
    no_meta_hostvars_host_invalid_json=dict(mygroup=dict(hosts=["myhost"])),  # triggers the legacy "slow" path which invokes the script with --host for each host
    # valid
    no_hosts=dict(_meta=dict(hostvars={})),
    no_profile=inventory,
    with_profile=inventory_with_profile,
    no_meta_hostvars=dict(mygroup=dict(hosts=["myhost"])),  # triggers the legacy "slow" path which invokes the script with --host for each host
    no_meta_hostvars_empty_host_result=dict(mygroup=dict(hosts=["myhost"])),  # triggers the legacy "slow" path which invokes the script with --host for each host
)
