...
from ansible.template.jinja_bits import _JinjaConstTemplate  # pylint: disable=unused-import
...
class TestDatatagTemplar(_TestDatatagTarget):
    ...
    def lazy_value(self, non_lazy_value, request: pytest.FixtureRequest, template_context):
        value_type = None  # DTFIX-MERGE: get from request when needed, can't easily add to the static fixture list- the generator cases are not being tested
        if value_type:
