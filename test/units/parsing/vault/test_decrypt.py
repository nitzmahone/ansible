...
from ansible.parsing.vault import ciphers, VaultLib, VaultSecret, parse_vaulttext_envelope
    ...
    for cipher_name in cipher_names:
        with importlib.resources.as_file(importlib.resources.files(decrypt_test_data).joinpath(cipher_name)) as path:
            if not (scenarios := list(path.glob('*.ciphertext'))):  # pragma: nocover
                params.append((cipher_name, "MISSING"))
            for scenario in scenarios:
                params.append((cipher_name, scenario.with_suffix('').name))
...
def test_fixed_decrypt(cipher_name: str, scenario: str) -> None:
    with importlib.resources.as_file(importlib.resources.files(decrypt_test_data).joinpath(cipher_name)) as path:
        ...
        assert parse_vaulttext_envelope(ciphertext_path.read_bytes())[2] == cipher_name.upper()
