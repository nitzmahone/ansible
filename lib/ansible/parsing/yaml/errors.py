...
class AnsibleYAMLParserError(AnsibleParserError):
    ...
    def handle_exception(cls, exception: Exception, src: str) -> t.NoReturn:
        if isinstance(exception, MarkedYAMLError):
            original_pos = AnsibleSourcePosition(src=src, line=exception.problem_mark.line + 1, col=exception.problem_mark.column + 1)
        else:
            original_pos = AnsibleSourcePosition(src=src)

        source_context = SourceContext.from_source_position(original_pos)
        ...
        # That's OK, users should "fix" their tab usage anyway -- at which point later error handling logic will hopefully find the real issue.
            ...
            source_context = SourceContext.from_source_position(original_pos.replace(col=tab_idx + 1))
