...
class _TemplateConfig:
    ...
    raise_on_trust_check_fail = False  # FIXME: make this configurable with multiple options (warn, warn_with_trace, error, ignore, etc)
...
class AnsibleUndefinedError(UndefinedError):
    ...
    # FIXME: give this class a name that reflects its usage as an internal-only flow control exception
...
def get_first_undefined_arg(args: c.Sequence, kwargs: dict[str, t.Any]) -> AnsibleUndefined | None:
    ...
    # FIXME: this may or may not need to be public API, move back to utils or once usage is wrapped in a decorator?
    for arg in itertools.chain(args, kwargs.values()):
        if type(arg) is AnsibleUndefined:  # pylint:disable=unidiomatic-typecheck
            # FIXME: global config for deprecation warning + return None
            return arg
