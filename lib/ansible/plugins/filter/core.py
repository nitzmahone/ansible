...
from jinja2.filters import do_default, do_map, do_select, do_selectattr, do_reject, do_rejectattr, pass_environment, sync_do_groupby
...
from ansible.template.jinja_common import AnsibleUndefinedError, get_first_undefined_arg
...
def get_encrypted_password(password, hashtype='sha512', salt=None, salt_size=None, rounds=None, ident=None):
    ...
    try:
        ...
        if unknown_passlib_hashtype:
            ...
            raise AnsibleError(f"The {hashtype!r} algorithm is not in the list of supported passlib algorithms: {choices}") from None
        raise
...
@pass_environment
def extract(environment: AnsibleEnvironment, item, container, morekeys=None):
    if morekeys is None:
        keys = [item]
...
@accept_undefined_args
@functools.wraps(do_default)
def wrapped_default(*args, **kwargs) -> t.Any:
    # Jinja's version of the filter accepts value, default_value, and boolean; it trips or returns undefined properly in all cases
    return do_default(*args, **kwargs)
