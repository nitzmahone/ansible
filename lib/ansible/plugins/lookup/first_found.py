...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        if te_action := JinjaCallContext.current()._te_invoking_action_name:
            ...
            terms = _omit_undefined(terms)  # recursively drop undefined values from terms for backwards compatibility
...
def _omit_undefined(o: t.Any) -> t.Any:
    match o:
        case dict():
            return {k: _omit_undefined(v) for k, v in o.items() if not isinstance(v, Undefined)}
        case list() | set() | tuple():
            return type(o)(_omit_undefined(v) for v in o if not isinstance(v, Undefined))
        case _:
            return o
