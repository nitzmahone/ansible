...
from ansible.template.jinja_common import UndefinedMarker, TruncationMarker
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        validation_result, new_module_args = self.validate_argument_spec(
            argument_spec=dict(
                ...
                var=dict(type=check_type_str_no_conversion),
        ...
        if verbosity <= self._display.verbosity:
            if raw_var_arg := new_module_args['var']:
                ...
                try:
                    results = self._templar.extend(marker_behavior=var_behavior).evaluate_expression(raw_var_arg)
