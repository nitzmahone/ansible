    ...
    def _create_role_list(self, fail_on_errors=True):
        ...
        for role, collection, role_path in (roles | collroles):
            ...
            try:
                ...
                # FIXME: ensure we have source position and __cause__ available, use warn instead?
            ...
            if 'error' in argspec:
                if fail_on_errors:
                    # FIXME: ensure we have source position and __cause__ available
...
def _format(string, *args):
    ...
    for style in args:
        ...
        if style not in ref_style and style.upper() not in STYLE and style not in C.COLOR_CODES:
            # FIXME: raise an apropos AnsibleError subclass and include position-tagged obj
    ...
    def _get_plugins_docs(self, plugin_type, names, fail_ok=False, fail_on_errors=True):
        ...
        for plugin in names:
            ...
            try:
                ...
                if fail_ok:
                    # FIXME: send the exception object to warning so we can show the traceback if enabled (once warning supports that)
    ...
    def add_fields(text, fields, limit, opt_indent, return_values=False, base_indent='', man=False):
        ...
        for o in sorted(fields):
            ...
            if not isinstance(required, bool):
                # FIXME: include position-tagged obj
        ...
        if doc.get('plainexamples', False):
            ...
            if isinstance(doc['plainexamples'], string_types):
                ...
                try:
                    ...
                    # FIXME: needs position-tagged obj ref
