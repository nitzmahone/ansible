...
def patch_dataclasses_is_type():
    ...
    try:
        real_is_type = dataclasses._is_type
    ...
    def _is_type(annotation, cls, a_module, a_type, is_type_predicate):
        match = dataclasses._MODULE_IDENTIFIER_RE.match(annotation)
        if match:
            ns = None
    ...
    _is_type._orig_impl = real_is_type  # stash this away to allow unit tests to undo the patch

    dataclasses._is_type = _is_type

    try:
        if _is_patch_needed():
            raise RuntimeError("patching had no effect")  # pragma: nocover
    except Exception as ex:  # pragma: nocover
        dataclasses._is_type = real_is_type
