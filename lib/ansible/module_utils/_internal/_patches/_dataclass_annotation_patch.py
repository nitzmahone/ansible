...
class DataclassesIsTypePatch(CallablePatch):
    ...
    @classmethod
    def _patched_impl(_cls, annotation, cls, a_module, a_type, is_type_predicate, *args, **kwargs) -> bool:
        """
    ...
    @classmethod
    def _get_patch(_cls):
        def func(annotation, cls, a_module, a_type, is_type_predicate) -> bool:
            match = dataclasses._MODULE_IDENTIFIER_RE.match(annotation)  # type: ignore[attr-defined]
            if match:
                ns = None
                module_name = match.group(1)
