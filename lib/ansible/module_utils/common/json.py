...
_common_module_response_types: frozenset[type[AnsibleSerializable]] = frozenset({
    ...
    # FIXME: any tag type *NOT* listed below should be moved out of module_utils so we're not giving users the ability to shoot themselves in the foot
    _Deprecated,
})
"""Types that must be supported for all Ansible module-to-controller serialization profiles."""


# FIXME: we probably need to hollow out this module, moving most to _internal and leaving whatever facade/public types behind that make sense
class _JSONSerializationProfile:
    ...
    # FIXME: should ensure_ascii be part of the profile, and set to False on some/all of them?
    ...
    def handle_key(cls, k: t.Any) -> t.Any:
        if not isinstance(k, str):  # FIXME: optimize this to use all known str-derived types in type map / allowed types
            raise TypeError(f'Key of type {type(k).__name__!r} is not JSON serializable by the {cls.profile_name!r} profile.')  # FIXME: better error message?
