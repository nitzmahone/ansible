...
_common_module_response_types: frozenset[type[AnsibleSerializable]] = frozenset({
    ...
    # DTFIX-U: any tag type *NOT* listed below should be moved out of module_utils so we're not giving users the ability to shoot themselves in the foot
    _Deprecated,
})
"""Types that must be supported for all Ansible module-to-controller serialization profiles."""


# DTFIX-U: we probably need to hollow out this module, moving most to _internal and leaving whatever facade/public types behind that make sense
class _JSONSerializationProfile:
    ...
    # DTFIX-U: should ensure_ascii be part of the profile, and set to False on some/all of them?
    ...
    def handle_key(cls, k: t.Any) -> t.Any:
        if not isinstance(k, str):  # DTFIX-U: optimize this to use all known str-derived types in type map / allowed types
            raise TypeError(f'Key of type {type(k).__name__!r} is not JSON serializable by the {cls.profile_name!r} profile.')  # DTFIX-U: better error message?
...
# DTFIX-U: use a frozen dataclass if it's not more expensive
class _WrappedValue:
    __slots__ = ('wrapped',)
...
class AnsibleJSONEncoder(json.JSONEncoder):
    ...
    def default(self, o):
        ...
        # DTFIX-U: optimize this; maybe direct-dispatch from a type mapping instead of using isinstance?
        # DTFIX-U: serialization "flavors" will affect this behavior
        if isinstance(o, Tripwire):
        ...
        elif isinstance(o, self._wrap_container_types):  # DTFIX-U: others, maybe sequence? can't use Iterable...
            value = [_WrappedValue(v) if isinstance(v, self._wrap_types) else v for v in o]
        elif isinstance(o, (datetime.date, datetime.datetime)):
            # date object
            value = o.isoformat()
        # DTFIX-U: we could've theoretically wrapped or returned just about any builtin; cheap way to catch all builtins?
