...
class Taggable:
    ...
    def _load_tags(self, attr, ds):
        ...
        if isinstance(ds, str):
            # FIXME: this allows each individual tag to be templated, but prevents the use of commas in templates, is that what we want?
            # FIXME: this can return empty tags (including a list of nothing but empty tags), is that correct?
            # FIXME: the original code seemed to attempt to preserve `ds` if there were no commas, but it never ran, what should it actually do?
            return [AnsibleTaggedObject.tag_copy(ds, item.strip()) for item in ds.split(',')]
