...
class Taggable:
    ...
    def _load_tags(self, attr, ds):
        ...
        if isinstance(ds, str):
            # DTFIX-U: this allows each individual tag to be templated, but prevents the use of commas in templates, is that what we want?
            # DTFIX-U: this can return empty tags (including a list of nothing but empty tags), is that correct?
            # DTFIX-U: the original code seemed to attempt to preserve `ds` if there were no commas, but it never ran, what should it actually do?
